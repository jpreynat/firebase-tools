...
function startAll(options, noGui = false) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (shouldStart(options, types_1.Emulators.FIRESTORE)) {
            ...
            utils.logLabeledBullet(types_1.Emulators.FIRESTORE, `For testing set ${clc.bold(`${constants_1.Constants.FIRESTORE_EMULATOR_HOST}=${firestoreAddr.host}:${firestoreAddr.port}`)}`);
        }
        if (shouldStart(options, types_1.Emulators.DATABASE)) {
            ...
            utils.logLabeledBullet(types_1.Emulators.DATABASE, `For testing set ${clc.bold(`${constants_1.Constants.FIREBASE_DATABASE_EMULATOR_HOST}=${databaseAddr.host}:${databaseAddr.port}`)}`);
        }
        ...
        if (!noGui && shouldStart(options, types_1.Emulators.GUI)) {
            const loggingAddr = constants_1.Constants.getAddress(types_1.Emulators.LOGGING, options);
