...
function releaseFunctions(context, options, uploadedNames, functionsInfo, attempt) {
    ...
    if (attempt > maxRetries) {
        logger.info("\n\n");
        utils.logWarning(clc.bold.yellow("functions: ") + `Failed to deploy all functions after ${maxRetries} times...`);
        return;
    }
    if (attempt > 0) {
    ...
    const legacySourceUrlTwo = "gs://" + "staging." + context.firebaseConfig.storageBucket + "/firebase-functions-source";
    ...
    return gcp.cloudfunctions
        ...
        _.chain(uploadedNames)
            ...
            if (migratingTrigger) {
                ...
                const runtime = options.runtime || _.get(existingFunction, "runtime", "nodejs6");
                utils.logBullet(clc.bold.cyan("functions: ") +
                    "updating " +
                    runtimeSelector.getHumanFriendlyRuntimeName(runtime) +
                    " function " +
        ...
        const hasQuotaError = failedCalls.some((error) => _.get(error, "context.response.statusCode") === 429);
        ...
        return _fetchTriggerUrls(projectId, successfulCalls, sourceUrl)
            ...
            if (failedDeployments.length > 0) {
                ...
                if (!allDeploymentsFailed &&
                    ...
                    !!options.retry &&
                    attempt <= maxRetries) {
                    const fullFunctionsNames = failedDeployments.map((name) => ["projects", projectId, "locations", "us-central1", "functions", name].join("/"));
                    const updatedOpts = Object.assign({}, options, {
                        only: failedDeployments
                            .map((name) => `functions:${name.split("-").join(".")}`)
                            .join(","),
                    });
                    return releaseFunctions(context, updatedOpts, fullFunctionsNames, functionsInfo, attempt + 1);
                }
